...
| `src/components/town/` | Illustrated Arkenfall hub, Blacksmith/Alchemist shop and recipe surfaces, and tavern rest presentation. |
...
| `src/components/devtools/` | Shared devtool contracts/launcher plus separate Enemy, Event, Adventure, and Item editors. |
...
- Standard consumables are `ConsumableItem` definitions. Use the shared typed effects and `useConsumable`; do not add item-ID branches for healing, Energy, fixed damage, or ordinary statuses.
...
- **Create Enemy** edits Physical Power, Spell Power, other combat stats, defenses, Hit/Dodge/Critical chances, Energy values, and an item drop table whose percentage rows roll independently for every defeated instance. Changing a numeric field or drop table for an existing enemy writes only that supported data directly to its canonical `src/game/content/enemies.ts` definition through the local Vite development server, preserving implemented abilities and behavior. New enemies remain drafts until implemented. Its **Add ability** flow creates any number of structured ability drafts containing a stable generated ID, name, Energy cost, cooldown, Melee/Ranged attack type, and free-form effect. It has no implicit default attack. Ability effects and behavior text are design input for later TypeScript implementation and are not executable on their own.
...
- **Adventure Editor** creates adventures, prerequisites, completion copy, ordered stages, and unlimited weighted combat/event possibilities. Selecting an adventure expands its stages as nested tabs in the left navigation; selecting a stage scrolls directly to its editor, and adjacent up/down controls change the canonical stage order without exposing IDs. Enemy and event pickers display readable names while internal adventure, stage, and possibility IDs stay hidden. Valid existing IDs are preserved; missing, invalid, or duplicate IDs are repaired automatically from the visible names before browser storage, Save, Copy, or Export. Enemy counts support repeated templates, combat entries configure XP and gold, and every stage has an item drop table with independent percentage chances. Its **XP Guide** lists the experience needed from the previous level and the cumulative total for every level through the level-50 cap; the table is derived from the live progression formula.
- **Item Editor** creates gear, set definitions, consumables, and ordinary **Other Items**. Every item has a non-negative Gold Cost used by event and Arkenfall merchants. **Sold in Arkenfall** assigns the item to Brunhilde's Blacksmith or Ray's Alchemist and may independently require one selected adventure to be completed before the item appears. **Craftable** assigns Blacksmith Crafting or Alchemist Brew plus any number of live item ingredients, whole-number quantities, and its own optional completed-adventure requirement. Adventure selectors display names while persisting stable IDs. These fields are executable immediately after Save; source validation rejects missing/self/duplicate ingredient references, empty recipes, unknown locations, and missing adventure references. Gear supports slot/hand classification, type-filtered gear artwork, the ordered Common/Uncommon/Rare/Epic/Legendary rarity tiers, attributes, defenses, both Powers, set membership, and non-executable special notes. The gear-art library contains five generated alternatives for every Head, Chest, Pants, Boots, Ring, Sword, Axe, Mace, Dagger, Wand, Shield, Tome, Staff, and Polearm category in addition to legacy icons; changing a slot or weapon kind immediately switches the picker to the matching category. Consumables compose any number of typed Health, Energy, next-turn regeneration, fixed-damage, and existing-status effects against self, selected target, or all enemies. Consumables and Other Items can select any generated artwork under `public/assets/items/`; the shared picker includes Metal Scrap plus twenty reusable crafting-material images spanning ores, fuel, wood, textiles, monster parts, alchemy ingredients, and elemental reagents. The selected URL is validated and saved with the canonical item. Other Items contain identity, rarity, price, description, and optional special notes but deliberately have no equip or combat-use behavior. Save validates and replaces `ITEMS` and `GEAR_SETS` in the canonical gear module; those standard fields therefore work in the live game without a Codex import. Special notes are retained in source/export but require implementation.
...
- Merchant and score-screen read-only gear inspection, merchant purchase/sold-out animation, centered rising-and-fading town purchase confirmation without an inline success panel, grouped duplicate loot, and the event Inventory/Equipment shortcut at mobile and desktop widths.
