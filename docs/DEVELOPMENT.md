...
- Adventure `theme` selects its dedicated adventure-card and combat-board backgrounds. The built-in values are `windsong_forest`, `arkenfall_highlands`, and `highfall_mountains`; `custom` reads the definition's required `cardImageUrl` and `combatBackgroundUrl`. Adventure Editor and local catalog validation support all four. Adventure `travelText` supplies the footsteps/loading copy whenever that adventure starts or advances to another stage; older editor drafts receive a theme-appropriate default during normalization.
