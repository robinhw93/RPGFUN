...
- Adventure `theme` selects its dedicated adventure-card and combat-board backgrounds. The current supported values are `windsong_forest`, `arkenfall_highlands`, and `highfall_mountains`; local catalog validation accepts all three. Adventure `travelText` supplies the footsteps/loading copy whenever that adventure starts or advances to another stage; older editor drafts receive a theme-appropriate default during normalization.
