...
- **Item Editor** creates gear, set definitions, consumables, and ordinary **Other Items**. Every item has a non-negative Gold Cost used by event and Arkenfall merchants. **Sold in Arkenfall** assigns the item to Brunhilde's Blacksmith, Ray's Alchemist, Mirelle's Tailor shop, Torren's Leatherworker shop, or Celestine's Jeweler shop and may independently require one selected adventure to be completed before the item appears. **Craftable** assigns the matching Blacksmith Crafting, Alchemist Brew, Tailoring, Leatherworking, or Jewelcrafting station plus any number of live item ingredients, whole-number quantities, and its own optional completed-adventure requirement. Adventure selectors display names while persisting stable IDs. These fields are executable immediately after Save; source validation rejects missing/self/duplicate ingredient references, empty recipes, unknown locations, and missing adventure references. Gear supports slot/hand classification, type-filtered gear artwork, the ordered Common/Uncommon/Rare/Epic/Legendary rarity tiers, attributes, defenses, both Powers, set membership, and non-executable special notes. The gear-art library contains five generated alternatives for every Head, Chest, Pants, Boots, Ring, Sword, Axe, Mace, Dagger, Wand, Shield, Tome, Staff, and Polearm category in addition to legacy icons; changing a slot or weapon kind immediately switches the picker to the matching category. Consumables compose any number of typed Health, Energy, next-turn regeneration, fixed-damage, and existing-status effects against self, selected target, or all enemies. Consumables and Other Items can select any generated artwork under `public/assets/items/`; the shared picker includes Metal Scrap plus twenty reusable crafting-material images spanning ores, fuel, wood, textiles, monster parts, alchemy ingredients, and elemental reagents. The selected URL is validated and saved with the canonical item. Other Items contain identity, rarity, price, description, and optional special notes but deliberately have no equip or combat-use behavior. Save validates and replaces `ITEMS` and `GEAR_SETS` in the canonical gear module; those standard fields therefore work in the live game without a Codex import. Special notes are retained in source/export but require implementation.
  ...
  Gear's live **Scaling Value** card converts standard numeric stats into a transparent comparison index, shows every scored contribution, and ranks the draft against other items for the same slot. One attribute point is the baseline value of 1; the centralized weights value Armor/Magic Resistance at 0.25, Physical/Spell Power at 0.75, Max Energy at 3, Energy Regeneration at 4, each percentage point of Hit/Dodge/Critical Strike Chance at 0.4, and Initiative at 0.25. Set bonuses, special notes, and advanced combat effects remain outside the numeric score and are explicitly flagged for manual evaluation.
