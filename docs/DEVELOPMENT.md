...
- **Item Editor** creates gear, set definitions, consumables, and ordinary **Other Items**. Every item has a non-negative Gold Cost used by event merchants. Gear supports slot/hand classification, existing gear art, rarity, attributes, defenses, both Powers, set membership, and non-executable special notes. Sets support any piece count, any number of thresholds, standard passive stats, and special notes. Consumables compose any number of typed Health, Energy, next-turn regeneration, fixed-damage, and existing-status effects against self, selected target, or all enemies. Other Items contain identity, rarity, price, description, and optional special notes but deliberately have no equip or combat-use behavior. Save validates and replaces `ITEMS` and `GEAR_SETS` in the canonical gear module; those standard fields therefore work in the live game without a Codex import. Special notes are retained in source/export but require implementation.
