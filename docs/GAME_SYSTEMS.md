...
Remaining Health carries from one stage to the next. Starting an adventure and moving between stages both show the animated footsteps transition first. A story combat then displays the selected Adventure Editor entry's Description; if that field is empty, the game falls back to generated enemy-count wording. Events do not show a generic discovery announcement after the footsteps. They move directly into their dedicated presentation, while dynamic testing and immediate event encounters use generated wording because they have no authored stage entry. Combat and its initiative presentation are created only after that introduction finishes. Completed adventure IDs are stored on the character and can satisfy another adventure's prerequisite.
