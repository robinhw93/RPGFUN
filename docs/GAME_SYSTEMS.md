...
The extended Brute section adds fifteen active abilities across Fire, Bleed, Armor/Guard, and holy paths. Divine Smite is Ranged and resolves as a target-bound holy strike; the other Brute abilities are Melee or self-targeted and use distinct, non-blocking VFX. Its reusable mechanics include Armor from Strength and percentage Armor, Burn-driven permanent Initiative, Health-restoration and Guard triggers, Guard derived from Armor, Physical Power, Magical Power, Max Health, and consumed status stacks, conditional Stun while Guarded, debuff cleansing, critical-strike status applications, immediate Burn/Bleed damage, and random multi-hit attacks.
...
The talent tree is classless. Wayfarer's Spark begins at the center, and the first four directions are Brute, Shadow, Arcanist, and Cultist. The live tree currently has 204 nodes: Shadow and Arcanist each have 79 including their class node, Brute has 44, and Cultist has 1. Shadow is the first complete branch. Arcanist extends from Arcane Mind into Fire, Frost, Lightning, and Arcane paths. Brute extends through Fire, Bleed, Armor/Guard, and holy-vigor paths ending in Furnace Breaker, Bloodbath, Shield Charge, and Divine Smite.
