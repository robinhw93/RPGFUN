...
Smite is a three-turn debuff. Whenever the player actually restores Health, including through combat consumables such as Healing Potions, Phoenix Heart, or Panic, every living enemy with Smite takes Magic damage based on 100% of that restored amount at the same presentation event. Overhealing does not increase Smite damage.
...
## Long-term progression and player records

The **Tower of Echoes** unlocks after The Throne Beyond Death is complete. A run begins at Floor 1 or the highest reached five-floor checkpoint. Existing enemy formations are reused with floor-owned Health, Power, defense, and level scaling; every fifth floor draws from boss formations. Clearing a floor grants `4 + floor × 2` Echo Essence. Health carries between floors, with a full restoration after each fifth-floor victory. Tower defeat and voluntary departure are safe: the story character is retained, the best cleared floor is updated, and the ten latest runs are stored.

The **Echo Forge** converts one unequipped Inventory gear item at a time into Echo Essence: Common 1, Uncommon 3, Rare 8, Epic 20, and Legendary 50. Reforging spends Essence and Gold to create a chosen set item, but only after that item's source adventure has been completed. Reforged gear uses normal acquisition and automatic empty-slot equipment rules.

The **Arkenfall Codex** records enemies when combat begins and items whenever they are acquired. Existing owned equipment and Inventory items are discovered during migration. Enemy and item entries remain hidden until found; sets show discovered-piece progress, while statuses are always available as a rules reference.

Every encounter owns a presentation-timed **Combat Report**. It records effective damage dealt and taken, effective player healing, player absorption, largest hit, Critical Hits, misses, Energy spent and gained, completed player turns, and player damage grouped by source. The report is available from story, Arena, and Tower results.

The **Hall of Fallen** uses a separate local store so permadeath can still delete the playable-character save. It retains up to twenty memorials with identity, level, final location and foe, equipment names, Arena best, Tower best, and date. Memorials grant no mechanical reward.

Presentation settings are stored separately from the character. Combat can be shown at 1×, 1.5×, or 2× speed; this divides presentation timers only. Reduced Motion suppresses non-essential animation and transitions.
