...
- The score screen animates experience once, shows gold, and allows access to the Character screen before continuing. Returning from the Character screen shows the completed experience result immediately instead of replaying it. When the reward grants at least one level and Attribute or Talent Points remain unspent, **View Character** changes to a continuously highlighted **Level up!** button and the journey button remains gray and disabled. Its tooltip lists whichever Attribute Points and Talent Points still need spending. After both are spent, the normal Character label and journey action return. The awarded-point summary is not duplicated in a separate panel.
