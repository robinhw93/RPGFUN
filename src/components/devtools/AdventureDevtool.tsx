...
export function AdventureDevtool({ onExit }: { onExit: () => void }) {
  ...
  const add = () => { const id = makeId("adventure"); const stageId = makeId("stage"); const adventure: AdventureDefinition = { id, name: "New Adventure", description: "", recommendedLevel: 1, theme: "windsong_forest", stages: [{ id: stageId, name: "Stage 1", dropTable: [], entries: [] }], completionTitle: "Adventure Complete", completionDescription: "" }; store.setDraft((draft) => ({ ...draft, adventures: [...draft.adventures, adventure] })); setSelectedId(id); setSelectedStageId(stageId); };
  const addStage = () => {
