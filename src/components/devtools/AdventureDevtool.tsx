...
import type { AdventureDefinition, AdventureStageDefinition, AdventureStageEntry } from "../../game/types";
import { ADVENTURE_DRAFT_STORAGE_KEY, copyJson, downloadJson, EditorShell, ensureInternalId, EVENT_DRAFT_STORAGE_KEY, localEnemies, makeId, NumberField, readExchange, saveLiveCatalog, TextField, useLocalDraft, useModalScrollLock, type AdventureExchange, type EventExchange } from "./shared";
...
export function AdventureDevtool({ onExit }: { onExit: () => void }) {
  ...
  const add = () => { const id = makeId("adventure"); const stageId = makeId("stage"); const adventure: AdventureDefinition = { id, name: "New Adventure", description: "", recommendedLevel: 1, theme: "windsong_forest", stages: [{ id: stageId, name: "Stage 1", entries: [] }], completionTitle: "Adventure Complete", completionDescription: "" }; store.setDraft((draft) => ({ ...draft, adventures: [...draft.adventures, adventure] })); setSelectedId(id); setSelectedStageId(stageId); };
  const addStage = () => {
    const stage: AdventureStageDefinition = { id: makeId("stage"), name: `Stage ${selected.stages.length + 1}`, entries: [] };
