...
import { getGearIconCategory, getGearIconChoices, resolveGearIconUrl } from "../GearSlotIcon";
import { copyJson, downloadJson, EditorShell, ensureInternalId, ITEM_DRAFT_STORAGE_KEY, JsonObjectField, makeId, NumberField, saveLiveCatalog, TextField, useLocalDraft, type ItemExchange } from "./shared";
...
function blankEffect(type: ConsumableEffect["type"]): ConsumableEffect {
  ...
  if (type === "remove_status") return { type, target: "self", status: "poison" };
...
function GearFields({ item, sets, allItems, onChange }: { item: GearItem; sets: GearSetDefinition[]; allItems: InventoryItem[]; onChange: (change: Partial<GearItem>) => void }) {
  ...
  return <>
    ...
    <div className="content-form-grid">
      ...
      {item.slot === "mainHand" && <><label><span>Weapon grip</span><select value={item.weaponEquipType ?? "oneHand"} onChange={(event) => onChange({ weaponEquipType: event.target.value as WeaponEquipType })}><option value="mainHand">Main hand only</option><option value="oneHand">One hand / either hand</option><option value="twoHand">Two hand</option></select></label><label><span>Weapon kind</span><select value={item.weaponKind ?? "sword"} onChange={(event) => onChange({ weaponKind: event.target.value as WeaponKind, iconUrl: undefined })}>{WEAPON_KINDS.filter((kind) => !["shield", "tome"].includes(kind)).map((kind) => <option key={kind}>{kind}</option>)}</select></label></>}
      {item.slot === "offHand" && <label><span>Off-hand kind</span><select value={item.weaponKind ?? "shield"} onChange={(event) => onChange({ weaponKind: event.target.value as WeaponKind, iconUrl: undefined })}><option value="shield">Shield</option><option value="tome">Tome / Focus</option><option value="dagger">Dagger</option><option value="wand">Wand</option></select></label>}
    ...
    <fieldset className="item-editor-section"><legend>Gear image: {iconCategoryLabel}</legend><div className="gear-image-picker">{iconChoices.map((url) => <button type="button" className={selectedIconUrl === url ? "selected" : ""} onClick={() => onChange({ iconUrl: url })} key={url}><img src={url} alt="" /><small>{url.split("/").pop()?.replace(".webp", "")}</small></button>)}</div></fieldset>
