...
import { ADVENTURES, ADVENTURE_EVENTS, ENEMIES, ITEMS } from "../game/data";
...
import type { AbilityRange, AdventureDefinition, AdventureEventChoice, AdventureEventDefinition, AdventureEventOutcome, AdventureEventOutcomeEffect, AdventureStageEntry, StatName, StatusEffectId } from "../game/types";
...
type EnemyEditableStats = Pick<EnemyDraft,
...
interface EventExchange { format: "arkenfall-events"; version: 2; events: AdventureEventDefinition[] }
interface AdventureExchange { format: "arkenfall-adventures"; version: 1; adventures: AdventureDefinition[] }

const EMPTY_OUTCOME: AdventureEventOutcome = { text: "", effects: [] };
...
async function saveLiveCatalog(kind: "events" | "adventures", exchange: EventExchange | AdventureExchange) {
...
function normalizeOutcome(outcome: AdventureEventOutcome): AdventureEventOutcome {
  return { text: outcome.text ?? "", effects: structuredClone(getAdventureEventOutcomeEffects(outcome)) };
}
function canonicalEventExchange(): EventExchange { return { format: "arkenfall-events", version: 2, events: Object.values(ADVENTURE_EVENTS).map((event) => structuredClone(event)) }; }
function normalizeEventExchange(exchange: EventExchange): EventExchange {
  return {
...
type OutcomePolarity = "positive" | "negative";
const POSITIVE_OUTCOME_OPTIONS: Array<{ type: AdventureEventOutcomeEffect["type"]; label: string }> = [
...
const NEGATIVE_OUTCOME_OPTIONS: Array<{ type: AdventureEventOutcomeEffect["type"]; label: string }> = [
...
const BUFF_OPTIONS = Object.values(STATUS_EFFECTS).filter((status) => status.kind === "buff");
const DEBUFF_OPTIONS = Object.values(STATUS_EFFECTS).filter((status) => status.kind === "debuff");

function blankOutcomeEffect(type: AdventureEventOutcomeEffect["type"]): AdventureEventOutcomeEffect {
  switch (type) {
...
function OutcomeEffectFields({ effect, polarity, enemies, onChange, onRemove }: {
...
function OutcomeFields({ title, polarity, outcome, enemies, onChange }: { title: string; polarity: OutcomePolarity; outcome: AdventureEventOutcome; enemies: Array<{ id: string; name: string }>; onChange: (outcome: AdventureEventOutcome) => void }) {
...
function localEnemies() {
  ...
  return [...new Map([
    ...Object.values(ENEMIES).map((enemy) => [enemy.id, { id: enemy.id, name: enemy.name }] as const),
    ...(stored?.enemies.map((enemy) => [enemy.id, { id: enemy.id, name: enemy.name }] as const) ?? []),
  ]).values()];
}
...
const XP_GUIDE_ROWS = Array.from({ length: MAX_LEVEL }, (_, index) => {
  const level = index + 1;
...
function XpGuideDialog({ onClose }: { onClose: () => void }) {
  useModalScrollLock();
