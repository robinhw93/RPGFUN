...
export function getDerivedStats(character: CharacterState): DerivedStats {
  ...
  return {
    ...
    physicalPower: Math.round((stats.strength * 0.5 + stats.agility * 0.25 + gearPhysicalPower) * (1 + features.passive.physicalPowerMultiplier)),
    magicalPower: Math.round((stats.intelligence * 0.5 + gearMagicalPower) * (1 + features.passive.magicalPowerMultiplier)),
