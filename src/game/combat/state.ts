...
export function ensureCombatState(combat: CombatState, character: CharacterState): CombatState {
  ...
  if (Array.isArray(combat.turnOrder) && combat.turnOrder.length > 0) {
    ...
    return {
      ...
      playerActionsThisTurn: Math.max(0, Math.round(combat.playerActionsThisTurn ?? (combat.playerActed ? 1 : 0))),
      consumablesUsedThisTurn: Math.max(0, Math.round(combat.consumablesUsedThisTurn ?? 0)),
      ...
      playerActionSurvivalPending: combat.playerActionSurvivalPending ?? false,
