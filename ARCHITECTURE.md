...
- `src/game/arena.ts` owns the safe Arena Champion session, attempt lock/reset, the whole-number Experience reward equal to 20% of actual damage, top-ten personal records, early trial completion, and restoration of pre-trial carried Health. `src/game/content/arena.ts` owns its hidden combat definition and artwork mapping without adding it to the story-adventure catalog.
...
`AdventureProgress` stores the active adventure ID, selected stage-entry ID, mode, carried Health, carried player ability cooldowns, event-roll result, queued next-combat statuses, a possible immediate event encounter, and combat independently. Story adventures use finite weighted stage definitions; `stageEntryId` locks in a randomly selected possibility so refreshing cannot reroll it. A victorious combat copies its remaining player cooldowns into the adventure before advancing; the next combat ticks those values once as its first player turn begins. Events preserve them, while a new, completed, fled, or abandoned adventure clears them.
...
- Completing the final stage clears active combat, records the adventure ID on the character, and marks the adventure completed. Before that ID is inserted, the arena rule checks whether this is the first completion and restores at most one arena attempt only in that case. The persistent ID keeps the green Completed treatment and unlocks replay rather than becoming terminal. Shared adventure reward scaling reduces every positive XP source during that replay to 10% and every combat gold reward to 50%, both rounded down. Replays never restore an arena attempt. Item drops retain their normal rules.
