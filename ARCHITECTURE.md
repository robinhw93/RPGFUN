...
- `src/components/ContentDevtools.tsx` is the compatibility facade for `src/components/devtools/`. The submodules separately own the launcher/shared draft contracts and Enemy, Event, Adventure, and Item editors. Existing-enemy numeric stat and drop-table edits call restricted local source-sync routes. Event, Adventure, and Item Save validate and replace their complete live catalogs while the local Vite server is running; enemy abilities and behavior remain draft/export input.
